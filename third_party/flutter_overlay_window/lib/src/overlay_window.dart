...
class FlutterOverlayWindow {
  ...
  /// FoxyCo: bring the host app to the foreground (bubble tap). Invoked on the
  /// overlay method channel — the same channel [resizeOverlay] uses — so it
  /// reaches native `OverlayService` directly from the overlay isolate.
  static Future<bool?> bringHostToFront() async {
    return await _overlayChannel.invokeMethod<bool?>('bringToFront');
  }
