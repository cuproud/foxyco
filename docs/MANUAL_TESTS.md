...
**Current candidate:** Play bundle build 110. Real-device validation is pending
for editable Home goals, startup/Home flows, manual ride recovery, long-hour
route parsing, cross-app OCR, overlay transparency, active-trip outcome
isolation, and diagnostic-only route matching.
...
| Q.9 | Select Uber with each supported app in turn, then trigger one real offer from each | Uber OCR turns on automatically; every selected app shows one correct pill/History row | [ ] build 101 |
...
| Q.13 | Leave each selected non-Uber app on its plain screen, then let Uber cover it; close Uber while a lower offer remains | Every selected app triggers the Uber probe, lower frames never clear its verdict, and a fresh lower verdict returns after Uber closes | [ ] build 101 |
| Q.14 | Capture Uber OCR as `$28.41`, `41 min`, `304 km`, followed by corrected `30.4 km`; also correct the saved test row in History | Impossible live data never reaches History; manual correction saves `30.4 km`, refreshes rates/session totals, and works for every platform | [ ] build 101 |
| Q.15 | Accept a ride, send navigation to Google Maps, then return to each selected driver app and copy Diagnostics | No grey square appears; recovery keeps Watching active; `overlay-native` logs show sanitized window/surface lifecycle | [ ] build 101 |
...
| Q.26 | Home → scroll below Last session; switch each goal period, edit each target, restart, then enable the largest app text | Each saved target survives restart; progress updates immediately; six-digit earnings and targets fit without clipping; only taken/completed History payouts count | [ ] build 110 |
...
| OCR.17 | Fresh install build 104, clear all history, then record 3–5 regular Uber offers plus Uber over each selected supported app | Every physical card maps to exactly one correct row; readable Uber uses Accessibility and cross-app probes stay rate-limited | [ ] |
