...
**Current candidate:** Play bundle build 100. Real-device validation is pending
...
| Q.9 | Select Uber, then trigger one real Uber/Lyft/Hopp offer | Uber OCR turns on automatically; all three show one correct pill/History row | [ ] build 100 |
...
| Q.13 | Leave Lyft/Hopp on its plain map, then let Uber cover it; close Uber while a lower offer remains | The map event detects Uber, lower frames never clear its verdict, and a fresh lower verdict returns after Uber closes | [ ] build 100 |
| Q.14 | Capture Uber OCR as `$28.41`, `41 min`, `304 km`, followed by corrected `30.4 km` | The impossible distance never reaches pill/History; only corrected `30.4 km` is shown and stored | [ ] build 100 |
| Q.15 | Accept a ride, send navigation to Google Maps, then return to each selected driver app | The fox bubble remains transparent with no grey square; if native health drops, Watching stays active and diagnostics show window recovery | [ ] build 100 |
...
| OCR.17 | Fresh install build 100, clear all history, then record 3–5 regular Uber offers plus Uber-over-Lyft/Hopp | Every physical card maps to exactly one correct row; readable Uber uses Accessibility and cross-app probes stay rate-limited | [ ] |
